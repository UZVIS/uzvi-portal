...
class PerformanceService:
    ...
    def get_cycle(
        db: Session,
        cycle_id: int
    ) -> Optional[models.ReviewCycle]:

        return (
            db.query(models.ReviewCycle)
            .filter(
                models.ReviewCycle.id == cycle_id
            )
            .first()
        )
    ...
    def create_goal(
        ...
        # Verify cycle exists
        cycle = PerformanceService.get_cycle(
            db,
            goal_data.cycle_id
        )
        ...
        if not cycle:
            raise ValueError(
                f"Review cycle {goal_data.cycle_id} not found"
            )
    ...
    def get_team_goals(
        ...
        query = (
            db.query(models.Goal)
            .filter(
                models.Goal.employee_id.in_(report_ids)
            )
        )
    ...
    def update_goal_status(
        ...
        goal = PerformanceService.get_goal(
            db,
            goal_id
        )
    ...
    def submit_self_assessment(
        ...
        existing = (
            db.query(models.SelfAssessment)
            .filter(
        ...
        # Update goal status
        goal.status = (
            models.GoalStatus.SELF_SUBMITTED
        )
    ...
    def get_self_assessment(
        db: Session,
        goal_id: int
    ) -> Optional[models.SelfAssessment]:
    ...
    def submit_manager_review(
        ...
        if not employee:
            raise ValueError(
                f"Employee {goal.employee_id} not found"
            )

        if employee.manager_id != reviewer_id:
            raise PermissionError(
                "Only the employee's manager can submit a review"
            )
        ...
        # Current goal is manager reviewed
        goal.status = (
            models.GoalStatus.MANAGER_REVIEWED
        )

        # ------------------------------------------------------
        # Check all goals for this employee in this cycle
        # ------------------------------------------------------

        cycle_goals = (
            PerformanceService.get_employee_goals(
                db,
                goal.employee_id,
                goal.cycle_id
            )
        ...
        all_reviewed = bool(cycle_goals) and all(
            g.id == goal.id or
            PerformanceService.get_manager_review(
                db,
                g.id
            ) is not None
        ...
        if all_reviewed:
            ...
            # All goals reviewed -> complete cycle goals
            for g in cycle_goals:
    ...
    def get_employee_review_status(
        ...
        for goal in goals:
            ...
            has_self = (
                PerformanceService.get_self_assessment(
                    db,
                    goal.id
                ) is not None
            )
            ...
            new_status = (
                PerformanceService.calculate_goal_status(
                    goal,
                    has_self,
                    has_manager
                )
        ...
        if completed == total_goals:
            ...
            overall_status = (
                models.GoalStatus.IN_PROGRESS
            )

        else:

            overall_status = (
                models.GoalStatus.NOT_STARTED
            )
    ...
    def get_org_review_status(
        ...
        if not cycle:
            raise ValueError(
                f"Review cycle {cycle_id} not found"
            )
        ...
        for emp in employees:
            ...
            status = (
                PerformanceService.get_employee_review_status(
                    db,
                    emp.employee_id,
                    cycle_id
                )
            ...
            # --------------------------------------------------
            # Employees without goals are shown in the table,
            # but are NOT counted in review-stage summary cards.
            # --------------------------------------------------

            if status.goals_count == 0:
                continue
            ...
            #
            # Self-assessment must already be submitted.
            # --------------------------------------------------
        ...
        return schemas.OrgReviewStatusResponse(
            ...
            pending_self_assessment_count=(
                pending_self_count
            ),
            pending_manager_review_count=(
