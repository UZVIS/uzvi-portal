...
#         super().__init__(
#             f"Logging {requested_hours}h would include overtime - only "
#             f"{remaining_normal_hours}h of normal time remain today."
#         )
...
#         .all()
#     )
#     hours_already_logged = sum(e.hours for e in same_day_entries)
#     normal_already_logged = sum(e.normal_hours for e in same_day_entries)
...
#         overtime_hours = 0.0
#     else:
#         if not data.confirm_overtime:
#             raise OvertimeConfirmationRequired(
#                 remaining_normal_hours=remaining_normal, requested_hours=data.hours
#             )
...
def get_time_entry(db: Session, entry_id: str) -> models.TimeEntry:
...
def list_pending_ot_entries(db: Session) -> List[models.TimeEntry]:
    """Admin/HR approval queue."""
    return db.query(models.TimeEntry).filter(models.TimeEntry.ot_status == "Pending").all()


def approve_ot(db: Session, entry_id: str, decided_by_role: str) -> models.TimeEntry:
    entry = get_time_entry(db, entry_id)
    if entry.ot_status != "Pending":
        raise InvalidOTTransitionError(f"Time entry {entry_id} has no pending overtime to approve.")
...
def reject_ot(db: Session, entry_id: str, decided_by_role: str) -> models.TimeEntry:
    entry = get_time_entry(db, entry_id)
    if entry.ot_status != "Pending":
        raise InvalidOTTransitionError(f"Time entry {entry_id} has no pending overtime to reject.")
...
def _effective_hours(entry: models.TimeEntry) -> float:
...
def compute_utilization(
    ...
    entries = list_time_entries(db, employee_id=employee_id, start_date=start_date, end_date=end_date)
    billable_hours = sum(_effective_hours(e) for e in entries if e.billable_flag)
    available_hours = capacity_hours_per_week * _weeks_in_period(start_date, end_date)
    utilization_pct = (billable_hours / available_hours) if available_hours else 0.0
...
def compute_personal_dashboard(
    ...
    for e in entries:
        ...
        iso_year, iso_week, _ = e.date.isocalendar()
        weekly_trend[f"{iso_year}-W{iso_week:02d}"] += effective
...
def compute_project_margin(db: Session, project_id: str) -> schemas.ProjectMargin:
    ...
    revenue = sum(_effective_hours(e) * billing_rate for e in entries if e.billable_flag)
    cost = sum(_effective_hours(e) * cost_rate for e in entries)
    margin = revenue - cost
    margin_pct = (margin / revenue) if revenue else None
