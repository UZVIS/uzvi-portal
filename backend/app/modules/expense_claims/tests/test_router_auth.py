...
def test_reject_over_threshold_returns_403_via_api(client, employees, db_session):
    """Regression test: the ₹25,000 threshold must block Manager from
    REJECTING a high-value claim via the API too, not just approving it -
    this was previously a bug where reject_claim had no threshold check
    at all, letting a Manager reject any claim regardless of amount."""
    service.create_category(
        db_session, schemas.ExpenseCategoryCreate(category_id="C9", name="Big", cap_amount=None)
    )
    service.create_claim(
        db_session,
        schemas.ExpenseClaimCreate(
            claim_id="CLR9", employee_id="EMP1", category_id="C9", amount=30000, date=date(2026, 1, 5)
        ),
    )
    res = client.post("/expenses/claims/CLR9/reject", headers={"X-Employee-Id": "MGR1"})
    assert res.status_code == 403


def test_admin_can_reject_over_threshold_via_api(client, employees, db_session):
    service.create_category(
        db_session, schemas.ExpenseCategoryCreate(category_id="C10", name="Big2", cap_amount=None)
    )
    service.create_claim(
        db_session,
        schemas.ExpenseClaimCreate(
            claim_id="CLR10", employee_id="EMP1", category_id="C10", amount=30000, date=date(2026, 1, 5)
        ),
    )
    res = client.post("/expenses/claims/CLR10/reject", headers={"X-Employee-Id": "ADM1"})
    assert res.status_code == 200
