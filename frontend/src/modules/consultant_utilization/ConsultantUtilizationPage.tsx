...
export function ConsultantUtilizationPage() {
  ...
  async function handleLogHours(entry: { projectId: string; date: string; hours: number; billable: boolean; notes: string }) {
    ...
    if (saved.ot_status === "Pending") {
      setOtNotice(
        `This includes ${saved.overtime_hours}h of overtime, sent for Admin/HR approval. ` +
  ...
  return (
    <div className="cu-page">
      ...
      {otNotice && <p className="cu-ot-notice">{otNotice}</p>}
            ...
            <tbody>
              {recentEntries.map((entry) => (
                <tr key={entry.entry_id}>
                  ...
                  <td>
                    {entry.ot_status ? (
                      <span className={`cu-ot-badge cu-ot-badge--${entry.ot_status.toLowerCase()}`}>
                        {entry.ot_status}
                      </span>
                    ) : (
                      "—"
